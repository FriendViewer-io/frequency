...
void ImageComponent::bind_data() const {
   RenderComponent::bind_data();
   ...
   shader->set_uniform("image_dims",
                       vec2(tex_data->width(), tex_data->height()) * get_parent()->get_scale());

   shader->set_uniform("position", get_parent()->get_position() + center_offset);
